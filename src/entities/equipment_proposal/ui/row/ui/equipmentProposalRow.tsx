...
export const EquipmentProposalRow: React.FC<EquipmentProposalRowType> = ({
                                                                             application
}) => {
    const router = useRouter();
    const company = useStore($authStore);
    const toggleIsFavorite = useUnit(applicationModel.updateIsFavoriteEquipmentApplicationFx);
    ...
    const handleOnClickInFavorite = (application: applicationModel.IEquipmentApplication) => {
        if (application) {
    ...
    return (
        <div className={!isMobile ? "inline-flex mt-7" : "mt-7"}>
            ...
            <div
                ...
                onClick={() => router.push(`/applications/${application.id}`)}>
                <h4 className="text-base font-medium">{application.equipment.name}</h4>
                ...
                <p className="text-xs text-grey-40 mt-[6px]">
                    {`Дата изготовления: ${application.manufactureDate}`}
                </p>
                <p className="text-xs text-grey-40 mt-[6px]">
                    {application.address}
                </p>
