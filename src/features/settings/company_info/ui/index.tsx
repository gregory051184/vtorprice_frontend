        ...
        <form ref={formRef} className={className}>
            ...
            <div className="mt-[26px]">
                ...
                <div className="flex flex-col gap-[16px]">
                    <div className={classNames('flex gap-[16px]', s.block)}>
                        <BaseInput loading={loadingStore} onChange={inn.onChange} value={inn.store.$value}
                                   error={!!inn.store.$error} className="grow" placeholder="ИНН" required/>
                        <BaseInput loading={loadingStore} onChange={name.onChange} value={name.store.$value}
                                   error={!!name.store.$error} className="grow" placeholder="Название компании"
                                   required/>
                    ...
                    <div className={classNames('flex gap-[16px] items-center', s.nds)}>
                        ...
                        />
                        <div
                            className="bg-white h-[56px] self-stretch px-[15px] flex items-center rounded-[10px] shadow">
                            <Checkbox checked={withNds.store.$value} onChange={withNds.onChange} description="С НДС"/>
                        </div>
                    </div>
                    <BaseInput loading={loadingStore} onChange={description.onChange} value={description.store.$value}
                               error={!!description.store.$error} className="grow"
            ...
            <div className="flex flex-col gap-[16px]">
                <div className={classNames('flex gap-[16px]', s.blockV2)}>
                    <BaseInput loading={loadingStore} onChange={head_full_name.onChange}
                               value={head_full_name.store.$value} error={!!head_full_name.store.$error}
                               className="grow" placeholder="ФИО директора (лицо с правом подписи)"/>
                    <BaseInput loading={loadingStore} onChange={bank_name.onChange} value={bank_name.store.$value}
                               error={!!bank_name.store.$error} className="grow" placeholder="Наименование банка"/>
                </div>
                <div className={classNames('flex gap-[16px]', s.blockV2)}>
                    <BaseInput loading={loadingStore} onChange={payment_account.onChange}
                               value={payment_account.store.$value} error={!!payment_account.store.$error}
                               className="grow" placeholder="Рассчётный счёт"/>
                    <BaseInput loading={loadingStore} onChange={correction_account.onChange}
                               value={correction_account.store.$value} error={!!correction_account.store.$error}
                               className="grow" placeholder="Корреспондентский счёт"/>
                    <BaseInput loading={loadingStore} onChange={bic.onChange} value={bic.store.$value}
                               error={!!bic.store.$error} className="grow" placeholder="БИК"/>
                </div>
            </div>
        </form>
