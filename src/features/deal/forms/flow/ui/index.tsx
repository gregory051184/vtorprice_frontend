        ...
        if (data) {
            await updateDeal({
                ...
                data: {
                    ...
                    comment: `Спор от ${new Date().toLocaleDateString()}, открыл ${user.user?.company.name}`
                }
            });
            router.push('/profile/deals');
        }
            ...
            ))}
            {
                ...
                (
                    (
                        ...
                        <Tip>Сформируйте <Link className='text-grey-40' href={`/transport-new-application?from-deal=${id}`}>новую транспортную
                            заявку</Link> для того, чтобы логист сформировал для Вас предложение
                            Обсудите предложения логистов компании VtorPrice и выберите подходящее в разделе <Link
                                className='text-grey-40' href='/profile/logistics'>“Логистика”</Link> Вашего Личного
                            кабинета.
                        </Tip>
                    ) || (
                        //@ts-ignore
                        user?.user?.role?.id <= ROLE.MANAGER ?
                            <Tip>
                                <Link className='text-grey-40' href={`/transport-new-application?from-deal=${id}`}>Создайте
                                    транспортную заявку</Link>
                            </Tip> :
                            <Tip
                                isBlue>Ожидаем выбор логиста {buyerpaysshipping ? 'покупателем' : 'продавцом'}
                            </Tip>
                    )
                )
            }
