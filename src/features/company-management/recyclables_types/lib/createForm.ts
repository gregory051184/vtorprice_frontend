...
export const createRecyclablesFormStateManagement = (action: 1 | 2, sid: string) => {
    ...
    formRecyclablesServerData.watch(() => {
    ...
    const initialValue = {
        firstField: {
            ...
            price: '',
            //isDeleted: false,
            isDeleted: stokForCompaniesRecyclables[0],
            //applicationRecyclableStatus: applicationRecyclableStatusSelectValues[0],
            removable: false,
        },
    }
    const form = createDynamicForm<{
        ...
        price: string,
        isDeleted: ISelectValue | null,
        //isDeleted: boolean,
        //applicationRecyclableStatus: ISelectValue | null,
    }>({
        ...
        emptyValue: {
            ...
            isDeleted: stokForCompaniesRecyclables[0]
            //isDeleted: false,
            //applicationRecyclableStatus: null,
        },
        validators: {
            recyclables: (val) => validationPipe(val, isNotNull()),
            monthlyVolume: (val) => validationPipe(val, isNotEmptyString(), minLimit()),
            price: (val) => validationPipe(val, isNotEmptyString(), maxPriceLimit(), minLimit()),
            //applicationRecyclableStatus: (val) => validationPipe(val, isNotNull()),
        },
        sid,
    });
            ...
            if (source) {
                const recyclables = source.reduce((acc, el, num) => ({
                    ...
                    [el.id]: {
                        ...
                        isDeleted: {
                            id: !el.deleted ? 1 : 2,
                            label: !el.deleted ? stokForCompaniesRecyclables[0].label : stokForCompaniesRecyclables[1].label,
                            value: !el.deleted ? stokForCompaniesRecyclables[0] :
                                stokForCompaniesRecyclables[1],
                        },
                        // applicationRecyclableStatus: {
                        //     id: el.applicationRecyclableStatus.id,
                        //     label: el.applicationRecyclableStatus.label,
                        //     value: el.applicationRecyclableStatus,
                        // },
                        removable: num !== 0,
                    },
                }), {});
