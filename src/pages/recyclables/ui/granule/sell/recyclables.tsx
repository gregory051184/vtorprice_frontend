...
export const RecyclablesGranuleSellByCategory = () => {
    ...
    const recyclableCategoryVolume = (categoryId: number): number => {
        const filtered_apps = applications.filter(app => app?.recyclables?.id === categoryId &&
            app?.dealType?.id === dealTypeSelectValues[1]?.id && app.applicationRecyclableStatus?.id === applicationRecyclableStatusSelectValues[1]?.id);
    ...
    if (isMobile) {
        return (
            ...
            >
                <Head>
                    <title>{recyclables.length > 0 ? `${recyclables.map(rec => rec?.category?.name)[0]} продажа` : 'Гранула продажа'}</title>
                </Head>
                <Container>
                    <BackButton/>
                    <div>
                        ...
                        >
                            {filteredRecyclableCategory().map(item =>
                                    <SwiperSlide
                                        ...
                                        onClick={() => item?.totalVolume > 0 && router.push(`/applications/granule/sell/category/recyclable/${item.recyclable.id}`)}
                    ...
                    <div className='mt-7'>
                        <h3>{recyclables.length > 0 ? `Компании с объявлениями по ${recyclables.map(rec => rec?.category?.name)[0]} продажа` : ''}</h3>
                    ...
                    {recyclables.length > 0 &&
                        <div className='mt-10'>
                            <h3>{`Объявления по ${recyclables.map(rec => rec?.category?.name)[0]} продажа`}</h3>
            ...
            <Container>
                ...
                <div className='inline-flex w-full mt-6'>
                    <div className='w-5/6'>
                        ...
                        {recyclables.length > 0 &&
                            <div className='mt-10'>
                                <h3>{`Объявлениями по ${recyclables.map(rec => rec?.category?.name)[0]} продажа`}</h3>
                                <FullListOfApplicationsForMainPage></FullListOfApplicationsForMainPage>
                            </div>}
                    </div>
                    <div>
