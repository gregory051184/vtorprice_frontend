...
export const ApplicationsAndCompaniesByRecyclableWasteBuy = () => {
    ...
    const cityRecyclableCategoryVolume = (categoryId: number, cityId: number): number => {
        const filtered_apps = applications.filter(app => app.recyclables.id === categoryId && app.company.city.id === cityId &&
            app.dealType.id === dealTypeSelectValues[0].id && app.applicationRecyclableStatus.id === applicationRecyclableStatusSelectValues[0].id);
    ...
    if (isMobile) {
        return (
            ...
            >
                <Head>
                    <title>{recyclable?.id ? `Покупка отходы (${recyclable?.name})` : 'Покупка отходы'}</title>
                </Head>
                <Container>
                    <BackButton/>
                    <div className="mt-6">
                        <h2>{recyclable?.id ? `Покупка отходы (${recyclable?.name})` : 'Покупка отходы'}</h2>
                        <div className='w-auto ml-5 inline-flex mt-6'>
                            <Select
                        ...
                        <div className='mt-6'>
                            ...
                            <div className='mt-6'>
                                <Button
                                    ...
                                    type="micro" fullWidth
                                    mode="light">{!showAllCompaniesWithCities ? "Показать компании сортированные по городам" : "Скрыть компании"}
                                </Button>
    ...
    return (
        ...
        >
            <Head>
                <title>{recyclable?.id ? `Покупка отходов (${recyclable?.name})` : 'Покупка отходов'}</title>
            </Head>
            <Container>
                <BackButton/>
                <div className="inline-flex mt-6">
                    <h2>{recyclable?.id ? `Покупка отходов (${recyclable?.name})` : 'Покупка отходов'}</h2>
                ...
                {(recyclable?.id && filteredRecFixed().length > 0) &&
                    <div className='mt-10'>
                        <h3>{
                            //@ts-ignore
                            `Объявлениями по покупке ${recyclable?.name}`}</h3>
                        <FullListOfApplicationsForMainPage></FullListOfApplicationsForMainPage>
                    </div>}
            </Container>
        </AppShell>
