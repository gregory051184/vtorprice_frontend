...
export const RecyclablesWasteBuyByCategory = () => {
    ...
    if (isMobile) {
        return (
            ...
            >
                <Head>
                    <title>{recyclables.length > 0 ? `Отходы ${recyclables.map(rec => rec?.category?.name)[0]} покупка` : 'Отходы покупка'}</title>
                </Head>
                <Container>
                    <BackButton/>
                    <div>
                    ...
                    <div className="mt-6">
                        <h1>{recyclables.length > 0 ? `Отходы ${recyclables.map(rec => rec?.category?.name)[0]} покупка` : 'Отходы покупка'}</h1>
                    ...
                    {recyclables.length > 0 &&
                        <div className='mt-10'>
                            <h3>{`Объявления по отходам ${recyclables.map(rec => rec?.category?.name)[0]} покупка`}</h3>
