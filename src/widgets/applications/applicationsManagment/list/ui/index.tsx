...
} from "@box/features/applications-management/filters/applicationsModerationsFilter/ui";
import {
    ApplicationManagementRow,
    EquipmentApplicationManagementRow
} from "@box/entities/application/ui/rows/applicationManagementRow";
import React, {useState} from "react";
import {$applicationsNumber, $equipmentApplicationsNumber} from "@box/entities/application/model";
import {EquipmentCreateForm} from "@box/entities/application/ui/templates/equipmentForm";
                    ...
                    <div>
                        ...
                        </Table>
                        {
                            ...
                            <div className="flex gap-4 cursor-pointer items-center float-right">
                                <p className="text-grey-50 text-sm whitespace-nowrap">Заявок по сырью</p>
...
export const EquipmentApplicationsManagementList: React.FC<IWithClass> = ({
    ...
    const [createNewEquipment, setCreateNewEquipment] = useState<boolean>(false)
    ...
    return (
        <>
            <Button
                ...
                fullWidth
                mode="light"
                onClick={() => setCreateNewEquipment(!createNewEquipment)}
            >
                {
                    !createNewEquipment ? "Занести новое оборудование" : "Вернуться к списку заявок"
                }
            </Button>
            {createNewEquipment &&
                <EquipmentCreateForm className="mt-6y"/>
            }
            {!createNewEquipment &&
                <div className='mt-6'>
                    <EquipmentApplicationsManagementListFilters/>
            ...
            {
                (!createNewEquipment && applicationsCount > 0) &&
                <div className="flex gap-4 cursor-pointer items-center float-right">
