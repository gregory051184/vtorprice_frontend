...
import {$verificationsCompaniesNumber} from "@box/entities/companyVerification/model";
...
export const CompaniesVerificationsList: React.FC<IWithClass> = ({
    ...
    const companiesVerificationsCount = useStore($verificationsCompaniesNumber)
    return (
        <div className={className}>
            <div>
                ...
                {
                    companiesVerificationsCount > 0 &&
                    <div className="flex gap-4 cursor-pointer items-center float-right">
                        <p className="text-grey-50 text-sm whitespace-nowrap">Компаний на верификацию</p>

                        <p>{companiesVerificationsCount}</p>
                    </div>
                }
            </div>
