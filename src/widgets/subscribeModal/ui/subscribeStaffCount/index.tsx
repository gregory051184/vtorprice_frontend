...
export const SubscribeStaffCountModal = () => {
    const showSubscribe = useStore($showSubscribeMaxStaffCountModal)
    const setShowModal = useEvent(setShowSubscribeMaxCountOfStaffModal)
    ...
    return (
        ...
        <Modal title={subscribe ? `Ваша подписка ${subscribe?.subscribe?.name}` : 'Подписка отсутствует'}
            ...
            <p className="text-center text-grey-90">
                {subscribe ? `Количество сотрудников для Вашей подписки - ${subscribe?.subscribe?.staffCount} исчерпано, попробуйте приобрести подписку с большим количеством` : 'Чтобы продолжить купите подписку'}
            </p>
